...
searchButtonOpen.addEventListener('click', () => {
    ...
    document.body.style.overflow = 'hidden';
    searchInput.focus();
    searchInput.value.trim().length > 0 && searchModal.classList.add('search--active-content');
});
...
searchButtonClose.addEventListener('click', () => {
    ...
    searchModal.classList.remove('search--active-content');
});
...
searchInput.addEventListener('input', (item) => {
    if(item.target.value.trim().length > 0){
        searchModal.classList.add('search--active-content');
    }
    else{
        searchModal.classList.remove('search--active-content');
    }
...
productCard.forEach((item, index) => {
    ...
    item.addEventListener('mouseenter', () => {
        // if(productCardSlider.slides.length > 1) productCardSlider.slideNext();
    });
